...
def summarize_article(article_text: str, model: str = "gpt-4o") -> dict:
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "당신은 청취자와 공감하며 이야기 나누는 감성적인 뉴스 DJ입니다. "
                        "감성적인 위로보다는 해당 뉴스 주제와 연결된 DJ 멘트를 만들어주세요. "
                        "멘트는 지나치게 추상적이지 않고, 실제 뉴스 이슈에 대해 DJ가 생각을 나누거나 "
                        "청취자에게 질문을 던지는 자연스러운 톤이면 좋습니다."},
        ...
        # 정규표현식 기반 파싱
        summary_match = re.search(r"핵심 요약\s*:\s*(.+)", content)
        dj_ment_match = re.search(r"감성 멘트\s*:\s*(.+)", content)
